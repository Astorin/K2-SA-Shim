...
local function mech_anim(name, table, light)
  ...
  local base_path = "__k2-sa-shim__/graphics/entity/mech-armor/"
  local anim =
  {
    layers = {
...
for _, animation in pairs(data.raw.character.character.animations) do
  if animation.armors and flib_table.find(animation.armors, "power-armor-mk2") then
    local index = flib_table.find(animation.armors, "power-armor-mk3")
    flib_table.remove(animation.armors, index)
  end
  if animation.armors and flib_table.find(animation.armors, "power-armor-mk2") then
    local index = flib_table.find(animation.armors, "power-armor-mk4")
    flib_table.remove(animation.armors, index)
  end
end
...
{
  armors = {"kr-power-armor-mk3", "kr-power-armor-mk4"},
...
data.raw['character-corpse']['character-corpse'].armor_picture_mapping["kr-power-armor-mk3"] = #data.raw['character-corpse']['character-corpse'].pictures
data.raw['character-corpse']['character-corpse'].armor_picture_mapping["kr-power-armor-mk4"] = #data.raw['character-corpse']['character-corpse'].pictures
